...
const Motors = () => {
    ...
    const columns = [
        ...
        },
        {
            title: 'Action',
            render: (record) => <>
                <button className='btn btn-primary mr-10' onClick={() => navigate('/editmotor/' + record?.id)}><FontAwesomeIcon icon={faEdit} /></button>
                <button className='btn btn-danger' onClick={() =>{setSelectedId(record?.id);}} ><FontAwesomeIcon icon={faTrash} /></button>
            </>,
        }
    ];
